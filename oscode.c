#include <stdio.h>
int numbers[1000] = {0}; //Set all values to 0 int the given array namely numbers , use to check for duplicacy
struct Processes
{
    int processId[10];
    int burstTime[10];
...
void showProcesses(int nop)
{
    ...
    for(; i < nop; i++)
    {
        printf("P%d\t\t%d\t\t%d\t\t%d\n", processes.processId[i], processes.burstTime[i], processes.priority[i], processes.deadline[i]);
    }
}
